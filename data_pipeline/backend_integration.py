...
from api.audio_router import process_audio_file
...
class BackendAudioProcessor:
    ...
    async def process_audio_file_from_pipeline(
        ...
        try:
            ...
            async with async_session() as db:
                ...
                # Step 3: Use existing backend processing logic
                await process_audio_file(audio_record.id, stored_file_info["file_path"])
    ...
    async def get_processing_status(self, audio_id: str) -> Optional[dict]:
        """
        Get processing status using backend repository.
    ...
    async def _process_file_async(self, file_path: Path):
        ...
        if file_path.suffix.lower() in {'.mp3', '.m4a', '.wav', '.ogg', '.webm', '.aac', '.flac'}:
            ...
            if audio_id:
                ...
                # Optional: Send webhook notification (reuse existing notifier)
                try:
